...
export default function AtlasGrid({ searchQuery, activeCategory }: AtlasGridProps) {
  ...
  if (filteredData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] w-full text-white/20 p-8 border border-dashed border-white/5">
        <span className="text-xl mb-2 tracking-widest">ZERO_RESULTS</span>
        <span className="text-xs">ADJUST_QUERY</span>
      </div>
    )
  }
  ...
  return (
    <div className="w-full flex flex-col bg-black overflow-hidden select-none">
      {/* Table Header */}
      <div className="h-10 border-b border-white z-20 flex text-[9px] font-bold tracking-[0.2em] text-white uppercase sticky top-0 bg-black">
        <div className="grid grid-cols-[1.5fr_2fr_120px_100px_80px] w-full h-full items-center">
          ...
          <div className="px-4 border-r border-white/10 h-full flex items-center text-right justify-end">EFFICIENCY</div>
      ...
      <div className="max-h-[75vh] overflow-y-auto scrollbar-hide">
        {filteredData.map((item) => (
          <div key={item.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
            <div className="grid grid-cols-[1.5fr_2fr_120px_100px_80px] w-full min-h-[56px] items-stretch text-[11px]">
              
              {/* Command */}
              <div className="px-4 py-3 font-mono flex items-center border-r border-white/5">
                <span className="text-white/20 mr-2">LTL</span>
              ...
              {/* Instruction */}
              <div className="px-4 py-3 text-white/40 leading-relaxed border-r border-white/5 flex items-center italic">
                {item.fullInstruction}
              </div>

              {/* Tokens */}
              <div className="px-4 py-3 text-right flex items-center justify-end font-mono border-r border-white/5">
                <span className="line-through mr-2 opacity-10 text-[9px]">{item.standardTokens}</span>
              ...
              <div className="px-4 py-3 text-right flex items-center justify-end">
                <button 
                  className="text-[9px] border border-white/20 px-2 py-1 hover:bg-white hover:text-black hover:border-white transition-all opacity-0 group-hover:opacity-100 uppercase font-bold"
      ...
      {/* Simple Status */}
      <div className="h-6 flex justify-between items-center px-4 text-[8px] text-white/20 tracking-widest font-mono select-none">
        <div>DB_IDX: {LTL_DATABASE.length} // FILTERED: {filteredData.length}</div>
        <div className="animate-pulse">SYSTEM_STABLE // READY</div>
      </div>
    </div>
  )
}
